...
over the `ondewo-survey-api` submodule, and it is regenerated in full by `make build`.
...
- **Transitive dependencies.** `Google.Protobuf`, `Grpc.Net.Client`, `Grpc.Core.Api` and
  ...
  `google/api` descriptors the ONDEWO SURVEY API imports are taken from `Google.Api.CommonProtos`
  rather than generated a second time into this assembly, so they never collide with another
  Google library in your graph.
- **What else is in the assembly.** Nothing: `ondewo-survey-api` imports no other ONDEWO product's
  protos, so the package carries the `Ondewo.Survey` stubs of its two service protos
  (`survey.proto` and `fhir.proto`) and nothing else. It sits beside any other ONDEWO client
  package without a type clash.
...
```shell
git clone https://github.com/ondewo/ondewo-survey-client-csharp.git   ## Clone the repository
cd ondewo-survey-client-csharp                                        ## Change into the repo directory
...
// `package ondewo.survey;` becomes `Ondewo.<Pascal>` — check the `namespace` line at the top of
...
using var channel = GrpcChannel.ForAddress(
    "https://grpc-survey.ondewo.com:443",
...
| `generate_ondewo_protos`              | runs that image over `ondewo-survey-api/ondewo` and writes the library back into the repo  |
...
Bump `ONDEWO_SURVEY_VERSION` in the `Makefile`, add a `RELEASE.md` entry in the existing format under a
`## Release ONDEWO SURVEY Csharp Client <version>` heading — `build_gh_release` slices the release notes out by
...
Pushing a version tag (`2.0.0`, `2.0.0-rc.1` — the shape `make create_release_tag` writes) also triggers
