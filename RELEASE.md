...
## Release ONDEWO SURVEY Csharp Client 2.0.0
...
* Initial release of the ONDEWO SURVEY C# client. The package `Ondewo.Survey.Client` ships the
  ...
  under `api/` - 4 files in this release.
...
* `tests/` is an xUnit suite (81 tests) over the committed stubs — a reflection-driven half that
  ...
  concrete ONDEWO SURVEY cases and full coverage of `auth/`. `make test` gates on 100% line, branch and method
  coverage of the hand-written sources; `api/` is excluded from the metric but exercised by the suite.
* The SURVEY API is the one ONDEWO API declaring two services, `ondewo.survey.Surveys` and
  `ondewo.survey.FHIR`; protoc keeps the latter's all-caps spelling, so the generated client type is
  `FHIR.FHIRClient`.
